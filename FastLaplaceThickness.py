...
class FastLaplaceWallThickness:
    @staticmethod
    ...
    def solve_laplace(self):
        ...
        print(f"  ✓ Build matrix arrays: {self.timing_stats['build_matrix_arrays']:.3f}s")
        ...
        print(f"  ✓ Create sparse matrix: {self.timing_stats['create_sparse_matrix']:.3f}s")
        print(f"    Matrix: {n_unknowns:,} x {n_unknowns:,}, {len(data_arr):,} non-zeros")
        ...
        if PYAMG_AVAILABLE:
            ...
            self.timing_stats['solve_method'] = 'PyAMG'
            print(f"  ✓ Sparse solve (PyAMG): {time.time() - t_start:.3f}s")
        else:
            # Fallback to direct solver
            x = spsolve(A, rhs)
            self.timing_stats['solve_method'] = 'spsolve'
    ...
    def calculate_thickness(self):
        ...
        if NUMBA_AVAILABLE:
            ...
            spacing_array = np.array(self.spacing, dtype=np.float64)
            dims_array = np.array(self.dims, dtype=np.int64)
            
            for i in range(n_points):
                thicknesses[i] = trace_streamline_numba(
                    endo_points[i], dir_x, dir_y, dir_z, self.potential_field,
